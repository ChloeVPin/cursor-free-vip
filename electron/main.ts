...
ipcMain.handle('system:saveScreenshot', async (_, imageData: string) => {
  try {
    ...
    const filename = `onyx-pass-log-${timestamp}.png`
...
function getAccountsFilePath(): string {
  return join(getUserDocumentsPath(), 'OnyxPass', 'accounts.json')
}
...
ipcMain.handle('update:disable', async (event) => {
  ...
  try {
    ...
    try {
      ...
      await fs.writeFile(updaterPath, '# Auto-update disabled by Onyx Pass\n', 'utf-8')
